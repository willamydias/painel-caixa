...
import { Landmark, ArrowRight, ShieldCheck, Zap, TrendingUp, Lock, Mail, Sparkles, KeyRound, AlertCircle } from 'lucide-react';

export default function LoginPage() {
  const { login, loginWithGoogle, isAuthenticated, isAuthLoaded } = useUser();
  ...
  // Se já estiver autenticado, direciona diretamente para o dashboard
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.success) {
      ...
      setErrorMsg(res.error || 'Credenciais inválidas. Verifique o e-mail e a senha.');
    }
  };
    ...
    <div className="flex min-h-screen bg-[var(--bg-page)] text-[var(--text-main)]">
      ...
      <div className="flex flex-1 flex-col justify-between p-6 sm:p-12 lg:p-16 max-w-xl mx-auto w-full">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="block text-[10px] text-[var(--text-muted)] font-medium uppercase tracking-wider">
                Inteligência de Oportunidades CAIXA
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="my-auto py-8 space-y-6">
          <div>
            <div className="inline-flex items-center gap-1.5 rounded-full bg-[var(--color-primary)]/10 px-3 py-1 text-xs font-bold text-[var(--color-primary)] border border-[var(--color-primary)]/20 mb-3">
              <Sparkles className="h-3.5 w-3.5" /> Acesso Restrito a Investidores
            </div>
            <h1 className="text-2xl sm:text-3xl font-extrabold text-[var(--text-main)] tracking-tight">
              Acesse sua conta
            </h1>
            <p className="text-sm text-[var(--text-muted)] mt-2">
              Informe seu e-mail corporativo ou pessoal e sua senha para acessar a plataforma analítica.
            </p>
          </div>

          {/* Mensagem de Erro de Validação */}
          {errorMsg && (
            <div className="rounded-xl border border-rose-500/30 bg-rose-500/10 p-3.5 text-xs font-bold text-rose-600 flex items-center gap-2 animate-in fade-in duration-200">
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full flex items-center justify-center gap-3 rounded-xl border border-[var(--border-main)] bg-[var(--bg-card)] px-4 py-3 text-sm font-bold text-[var(--text-main)] hover:bg-[var(--bg-sub)] transition-all shadow-sm cursor-pointer"
          >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full flex items-center justify-center gap-2 rounded-xl bg-[var(--color-primary)] hover:bg-[var(--color-primary-hover)] text-white py-3.5 px-4 text-sm font-bold shadow-md shadow-[var(--color-primary)]/20 transition-all cursor-pointer disabled:opacity-50"
            >
          ...
          {/* Painel de Auxílio de Desenvolvimento (Atalhos Rápidos de Teste) */}
          <div className="rounded-2xl border border-[var(--border-main)] bg-[var(--bg-sub)] p-4 space-y-2 text-xs">
            <div className="flex items-center gap-1.5 font-bold text-[var(--text-main)]">
              <KeyRound className="h-4 w-4 text-[var(--color-primary)]" />
              <span>Contas de Desenvolvimento (Clique para preencher):</span>
            </div>
            <div className="flex flex-wrap gap-2 pt-1">
              <button
                type="button"
                onClick={() => fillDevAccount('willamy.dias@gmail.com')}
                className="rounded-lg border border-[var(--border-main)] bg-[var(--bg-card)] px-2.5 py-1 text-[11px] font-bold text-[var(--text-main)] hover:border-[var(--color-primary)] transition-all cursor-pointer"
              >
                Willamy (Admin)
              </button>
              <button
                type="button"
                onClick={() => fillDevAccount('wagner.investor@gmail.com')}
                className="rounded-lg border border-[var(--border-main)] bg-[var(--bg-card)] px-2.5 py-1 text-[11px] font-bold text-[var(--text-main)] hover:border-[var(--color-primary)] transition-all cursor-pointer"
              >
                Wagner (Investidor)
              </button>
              <button
                type="button"
                onClick={() => fillDevAccount('ana.juridico@gmail.com')}
                className="rounded-lg border border-[var(--border-main)] bg-[var(--bg-card)] px-2.5 py-1 text-[11px] font-bold text-[var(--text-main)] hover:border-[var(--color-primary)] transition-all cursor-pointer"
              >
                Dra. Ana (Analista)
              </button>
        ...
        <div className="text-xs text-[var(--text-muted)] text-center sm:text-left">
          © 2026 Painel Imobiliário. Todos os direitos reservados.
        </div>
      </div>

      {/* Coluna Direita: Showroom editorial & proposta de valor */}
      <div className="hidden lg:flex flex-1 flex-col justify-between bg-slate-900 text-white p-12 relative overflow-hidden border-l border-[var(--border-main)]">
        ...
        <div className="relative z-10">
          <span className="inline-flex items-center gap-2 rounded-full bg-teal-500/20 px-3 py-1 text-xs font-bold text-teal-300 border border-teal-500/30">
            <Zap className="h-3.5 w-3.5 text-teal-400" /> Plataforma SaaS Multiusuário v2.5
          </span>
        </div>
        ...
        <div className="relative z-10 text-xs text-slate-400 flex items-center justify-between border-t border-slate-800 pt-6">
          <span>© 2026 Painel Imobiliário. Todos os direitos reservados.</span>
          <span>Status: Autenticação Ativa</span>
        </div>
      </div>
    </div>
  );
